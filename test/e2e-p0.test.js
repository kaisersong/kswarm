...
function currentRun(hub, projectId, taskRef) {
  return hub.getBoard(projectId)?.getTask(taskRef)?.activeRunId;
}
...
scenario('1. 智能分配 — 按能力匹配 + 负载均衡完整流程', () => {
  ...
  assert(dispatchResult.dispatched.length === 3, `同一 agent 同轮只派 1 个任务: ${dispatchResult.dispatched?.length}`);
  assert(dispatchResult.skipped.filter(s => s.reason === 'agent_busy').length === 2, '同 agent 余下任务保持 pending');
...
scenario('2. Watchdog — 检测超时 → 重试 → 最终失败 完整生命周期', () => {
  ...
  // worker-a 接受 t1 并开始
  const wdT1Run = currentRun(hub, 'e2e-wd-1', 'wd-t1');
  hub.handleAcceptTask('e2e-wd-1', 'wd-t1', 'worker-a', wdT1Run);
  hub.handleProgress('e2e-wd-1', 'wd-t1', 'started', 'worker-a', wdT1Run);
  assert(board.getTask('wd-t1').status === 'in_progress', 't1 进入 in_progress');

  // worker-b 正常完成 t2
  const wdT2Run = currentRun(hub, 'e2e-wd-1', 'wd-t2');
  hub.handleAcceptTask('e2e-wd-1', 'wd-t2', 'worker-b', wdT2Run);
  hub.handleProgress('e2e-wd-1', 'wd-t2', 'started', 'worker-b', wdT2Run);
  hub.handleSubmitResult('e2e-wd-1', 'wd-t2', { summary: 'done' }, 'worker-b', wdT2Run);
  ...
  assert(watchdog.getRetryCount('e2e-wd-1__wd-t1') === 2, '重试计数=2');
...
await scenario('4. QoderCLI — agent-store 注册 + detectCLIs 集成', async () => {
  ...
  // 验证 CLI 列表包含关键运行时；允许后续新增 CLI 类型
  assert(known.length >= 12, `已知 CLI 数量不少于 12: ${known.length}`);
...
scenario('5. 完整生命周期 — 智能分配 → 执行 → 超时恢复 → 交付聚合', () => {
  ...
  const fullT1Run = currentRun(hub, 'e2e-full-1', 'full-t1');
  hub.handleAcceptTask('e2e-full-1', 'full-t1', 'coder-1', fullT1Run);
  hub.handleProgress('e2e-full-1', 'full-t1', 'started', 'coder-1', fullT1Run);
  hub.handleSubmitResult('e2e-full-1', 'full-t1', { summary: 'login API done', artifacts: ['full-t1-report.md'] }, 'coder-1', fullT1Run);
  ...
  assert(dispatch2.dispatched[0] === 'e2e-full-1__full-t2', `派发了 t2: ${dispatch2.dispatched[0]}`);

  // Step 6: tester-1 接受但超时
  const fullT2Run = currentRun(hub, 'e2e-full-1', 'full-t2');
  hub.handleAcceptTask('e2e-full-1', 'full-t2', 'tester-1', fullT2Run);
  hub.handleProgress('e2e-full-1', 'full-t2', 'started', 'tester-1', fullT2Run);
  ...
  const fullT2RetryRun = currentRun(hub, 'e2e-full-1', 'full-t2');
  hub.handleAcceptTask('e2e-full-1', 'full-t2', 'tester-1', fullT2RetryRun);
  hub.handleProgress('e2e-full-1', 'full-t2', 'started', 'tester-1', fullT2RetryRun);
  hub.handleSubmitResult('e2e-full-1', 'full-t2', { summary: 'tests written', artifacts: ['full-t2-report.md'] }, 'tester-1', fullT2RetryRun);
...
scenario('7. Watchdog 安全性 — 不干扰正常进行中的任务', () => {
  ...
  // t1: 刚开始（不超时）
  const safeT1Run = currentRun(hub, 'e2e-safe-1', 'safe-t1');
  hub.handleAcceptTask('e2e-safe-1', 'safe-t1', 'w1', safeT1Run);
  hub.handleProgress('e2e-safe-1', 'safe-t1', 'started', 'w1', safeT1Run);

  // t2: 超时
  const safeT2Run = currentRun(hub, 'e2e-safe-1', 'safe-t2');
  hub.handleAcceptTask('e2e-safe-1', 'safe-t2', 'w2', safeT2Run);
  hub.handleProgress('e2e-safe-1', 'safe-t2', 'started', 'w2', safeT2Run);
  board.getTask('safe-t2').updatedAt = Date.now() - 700_000;

  // t3: 已提交等待 review（不应被 watchdog 干扰）
  const safeT3Run = currentRun(hub, 'e2e-safe-1', 'safe-t3');
  hub.handleAcceptTask('e2e-safe-1', 'safe-t3', 'w3', safeT3Run);
  hub.handleProgress('e2e-safe-1', 'safe-t3', 'started', 'w3', safeT3Run);
  hub.handleSubmitResult('e2e-safe-1', 'safe-t3', { summary: 'done' }, 'w3', safeT3Run);
